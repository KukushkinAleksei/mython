...
// Специальный класс-обёртка, предназначенный для хранения объекта в
// Mython-программе
class ObjectHolder {
 public:
  ...
  // Возвращает указатель на объект типа T либо nullptr, если внутри
  // ObjectHolder не хранится объект данного типа
  template <typename T>
...
class ValueObject : public Object {
 public:
  ValueObject(
      T v)  // NOLINT(google-explicit-constructor,hicpp-explicit-conversions)
      : value_(v) {}
...
// Для отличных от нуля чисел, True и непустых строк возвращается true. В
...
class Class : public Object {
 public:
  // Создаёт класс с именем name и набором методов methods, унаследованный от
  // класса parent Если parent равен nullptr, то создаётся базовый класс
  explicit Class(std::string name, std::vector<Method> methods,
                 const Class* parent);

  // Возвращает указатель на метод name или nullptr, если метод с таким именем
  // отсутствует
...
class ClassInstance : public Object {
 public:
  ...
  /*
   * Если у объекта есть метод __str__, выводит в os результат, возвращённый
   * этим методом. В противном случае в os выводится адрес объекта.
   */
  void Print(std::ostream& os, Context& context) override;
   ...
   * Если ни сам класс, ни его родители не содержат метод method, метод
   * выбрасывает исключение runtime_error
   */
  ObjectHolder Call(const std::string& method,
                    const std::vector<ObjectHolder>& actual_args,
                    Context& context);

  // Возвращает true, если объект имеет метод method, принимающий argument_count
  // параметров
...
/*
 * Возвращает true, если lhs и rhs содержат одинаковые числа, строки или
 * значения типа Bool. Если lhs - объект с методом __eq__, функция возвращает
 * результат вызова lhs.__eq__(rhs), приведённый к типу Bool. Если lhs и rhs
 * имеют значение None, функция возвращает true. В остальных случаях функция
 * выбрасывает исключение runtime_error.
 *
...
/*
 * Если lhs и rhs - числа, строки или значения bool, функция возвращает
 * результат их сравнения оператором <. Если lhs - объект с методом __lt__,
 * возвращает результат вызова lhs.__lt__(rhs), приведённый к типу bool. В
 * остальных случаях функция выбрасывает исключение runtime_error.
 *
...
// Простой контекст, в нём вывод происходит в поток output, переданный в
// конструктор
