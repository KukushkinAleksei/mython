...
template <typename T>
void AssertObjectValueEqual(const ObjectHolder& obj, const T& expected,
                            const string& msg) {
  ostringstream one;
...
void TestVariable() {
  ...
  ASSERT_THROWS(VariableValue("unknown"s).Execute(closure, context),
                std::runtime_error);
...
void TestAssignment() {
  ...
  Assignment assign_y("y"s,
                      make_unique<StringConst>(runtime::String("Hello"s)));
...
void TestFieldAssignment() {
  ...
  const auto* subobject =
      object.Fields().at("y"s).TryAs<runtime::ClassInstance>();
...
void TestStringify() {
  ...
  {
    auto result =
        Stringify(make_unique<NumericConst>(57)).Execute(empty, context);
    ASSERT_OBJECT_VALUE_EQUAL(result, "57"s);
    ASSERT(result.TryAs<runtime::String>());
  }
  {
    auto result =
        Stringify(make_unique<StringConst>("Wazzup!"s)).Execute(empty, context);
  ...
  }
  {
    ...
    auto result =
        Stringify(make_unique<NewInstance>(cls)).Execute(empty, context);
  ...
  }
  {
    ...
    runtime::Closure closure{
        {"x"s, ObjectHolder::Own(runtime::ClassInstance{cls})}};

    std::ostringstream expected_output;
    expected_output << closure.at("x"s).Get();

    Stringify str(make_unique<VariableValue>("x"s));
    ASSERT_OBJECT_VALUE_EQUAL(str.Execute(closure, context),
                              expected_output.str());
  }
  {
...
void TestBadAddition() {
  ...
  ASSERT_THROWS(
      Add(make_unique<StringConst>("4"s), make_unique<NumericConst>(42))
          .Execute(empty, context),
      std::runtime_error);
  ASSERT_THROWS(Add(make_unique<None>(), make_unique<StringConst>("4"s))
                    .Execute(empty, context),
                std::runtime_error);
  ASSERT_THROWS(
      Add(make_unique<None>(), make_unique<None>()).Execute(empty, context),
      std::runtime_error);
...
void TestFields() {
  ...
  methods.push_back(
      {"value"s,
       {},
       {make_unique<VariableValue>(vector<string>{"self"s, "value"s})}});
  methods.push_back(
      {"add"s,
       {"x"s},
...
void TestOr() {
  auto test_or = [](bool lhs, bool rhs) {
    ...
    runtime::DummyContext context;
    ASSERT_EQUAL(
        runtime::Equal(or_statement.Execute(closure, context),
...
void TestAnd() {
  auto test_and = [](bool lhs, bool rhs) {
    ...
    runtime::DummyContext context;
    ASSERT_EQUAL(
        runtime::Equal(and_statement.Execute(closure, context),
                       ObjectHolder::Own(runtime::Bool(true)), context),
        lhs && rhs);
  };
...
void TestNot() {
  auto test_not = [](bool arg) {
    ...
    runtime::DummyContext context;
    ASSERT_EQUAL(
        runtime::Equal(not_statement.Execute(closure, context),
                       ObjectHolder::Own(runtime::Bool(true)), context),
        !arg);
  };
