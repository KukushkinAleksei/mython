...
ObjectHolder VariableValue::Execute(Closure& closure, Context& /*context*/) {
  ...
  for (const string id : dotted_ids_) {
    ...
    runtime::ClassInstance* class_instance =
        cur_obj_ptr->second.TryAs<runtime::ClassInstance>();
...
ObjectHolder Div::Execute(Closure& closure, Context& context) {
  ...
  if (ptr_lhs_n && ptr_rhs_n) {
    auto l_num = ptr_lhs_n->GetValue();
    auto r_num = ptr_rhs_n->GetValue();
    if (r_num == 0) throw runtime_error("zero divizion");
...
ReturnExeption::ReturnExeption(const runtime::ObjectHolder& obj) : obj_(obj) {}

runtime::ObjectHolder ReturnExeption::GetValue() { return obj_; }
...
ObjectHolder Comparison::Execute(Closure& closure, Context& context) {
  bool result = cmp_(lhs_->Execute(closure, context),
...
NewInstance::NewInstance(const runtime::Class& class_,
                         std::vector<std::unique_ptr<Statement>> args)
    : class_inst_(class_), args_(move(args)) {}
