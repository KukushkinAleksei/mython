...
Class::Class(std::string name, std::vector<Method> methods, const Class* parent)
    : parent_(parent), name_(move(name)), methods_(move(methods)) {
  if (parent_) {
    name_to_method_.insert(parent_->name_to_method_.begin(),
                           parent_->name_to_method_.end());
  }
...
const Class* Class::GetParent() const { return parent_; }
...
bool CmpOpImpl(const ObjectHolder& lhs, const ObjectHolder& rhs,
               const Comparator& cmp, bool& result) {
...
bool Equal(const ObjectHolder& lhs, const ObjectHolder& rhs, Context& context) {
  ...
  bool result = false;
  if (CmpOpImpl<Number>(lhs, rhs, equal_to<int>(), result)) return result;

  if (CmpOpImpl<Bool>(lhs, rhs, equal_to<bool>(), result)) return result;

  if (CmpOpImpl<String>(lhs, rhs, equal_to<string>(), result)) return result;

  auto l_ptr_class_inst = lhs.TryAs<ClassInstance>();
  const int EQ_METHOD_ARGS_COUNT = 1;
  if (l_ptr_class_inst &&
      l_ptr_class_inst->HasMethod(EQ_METHOD, EQ_METHOD_ARGS_COUNT)) {
...
bool Less(const ObjectHolder& lhs, const ObjectHolder& rhs, Context& context) {
  bool result = false;
  if (CmpOpImpl<Number>(lhs, rhs, less<int>(), result)) return result;

  if (CmpOpImpl<Bool>(lhs, rhs, less<bool>(), result)) return result;

  if (CmpOpImpl<String>(lhs, rhs, less<string>(), result)) return result;

  auto l_ptr_class_inst = lhs.TryAs<ClassInstance>();
  const int LT_METHOD_ARGS_COUNT = 1;

  if (l_ptr_class_inst &&
      l_ptr_class_inst->HasMethod(LT_METHOD, LT_METHOD_ARGS_COUNT)) {
